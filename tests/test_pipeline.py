...
def test_pipeline_accepts_once(tmp_path):
    pipeline = LeadPipeline(MockFacebookCollector(), KeywordFilter(["cần gia sư"]), ProcessedPostStore(f"sqlite:///{tmp_path}/test.db"), GoogleSheetsWriter())
...
def test_offer_post_is_not_a_lead(tmp_path):
    ...
    pipeline = LeadPipeline(OfferCollector(), KeywordFilter(["cần gia sư"]), ProcessedPostStore(f"sqlite:///{tmp_path}/test.db"), GoogleSheetsWriter())
...
def test_pipeline_deduplicates_identical_content_across_post_ids(tmp_path):
    ...
    class DuplicateCollector(BaseCollector):
        def collect(self, source):
            return [
                RawPost(post_id="first", source=source, content="Cần gia sư Toán lớp 8."),
                RawPost(post_id="second", source=source, content="Cần   gia sư Toán lớp 8."),
            ]

    pipeline = LeadPipeline(DuplicateCollector(), KeywordFilter(["cần gia sư"]), ProcessedPostStore(f"sqlite:///{tmp_path}/test.db"), GoogleSheetsWriter())
...
def test_pipeline_exports_accepted_leads_as_one_batch(tmp_path):
    ...
    class TwoPosts(BaseCollector):
        def collect(self, source):
            return [RawPost(post_id=str(i), source=source, content=f"Cần gia sư Toán lớp {i}.") for i in (7, 8)]

    class BatchWriter:
        def __init__(self): self.batches = []
        def append_many(self, leads): self.batches.append(leads)

    writer = BatchWriter()
    pipeline = LeadPipeline(TwoPosts(), KeywordFilter(["cần gia sư"]), ProcessedPostStore(f"sqlite:///{tmp_path}/test.db"), writer)
